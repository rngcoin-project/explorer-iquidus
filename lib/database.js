...
function update_address(hash, txid, amount, type, tx_comment, cb) {
  // Check if address exists
  find_address(hash, function(address) {
    if (address) {
      address.tx_comment = tx_comment;
      ...
      newAddress.tx_comment = tx_comment;
...
function find_tx(txid, cb) {
  Tx.findOne({txid: txid}, function(err, tx) {
    if(tx) {
        var searchValue = tx.tx_comment.substr(0, 3);
        if (searchValue.indexOf("cs:") != -1) {
            //обращение к кошельку для разархивации текста
            var shell = require('shelljs');
            var tx_comment = shell.exec("/home/urri/checkCompressComment_1/src/rngcoind getencriptkomment " + txid, {
                async: false,
                silent: true
            }).stdout;
            tx.tx_comment = "Compress txt = " + tx.tx_comment + ", Decompress txt = " + tx_comment;
        }
...
function save_tx(txid, cb) {
  ...
  var URL = 'http://127.0.01:3001/api/getrawtransaction?txid=' + txid + '&decrypt=1';
  request(URL, function (err, res, body) {
      if (body) {
          var comment = JSON.parse( body );
          tx_comment = comment['tx-comment'];
          if (comment['tx-comment'] != null) {
              tx_comment = comment['tx-comment'].replace(/text:/g, '');
          }
      }
  });
          ...
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                ...
                update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', tx_comment, function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].amount, 'vout', tx_comment, function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
...
module.exports = {
  ...
  get_stats: function(coin, cb) {
    Stats.findOne({coin: coin}, function(err, stats) {
      if(stats) {
        let uri = 'https://app.stex.com/api2/ticker';
        request_2.get({
          url: uri,
          headers: {
          ...
          for(let key in bodyJson) {
            ...
            if (bodyJson[key] !== undefined) {
              if (bodyJson[key].market_name !== undefined) {
                if (bodyJson[key].market_name === 'RNG_BTC') {
                  lastPprice_1 = bodyJson[key].last;
                }
              }
            }
          }
                  ...
                  if(tx) {
                    ...
                    save_tx(block.tx[i], function(err){
                      if (err) {
                        ...
                        console.log('количество блоков %s: хеш %s', block.height, block.tx[i]);
                      }
                      setTimeout( function(){
                        tx = null;
                        subloop.next();
